...
@Component({
            ...
            <div id="recent-loans" class="recent-list">
              @for (loan of recentLoans(); track loan.id) {
                <div class="recent-item">
                  ...
                  <span class="recent-item-amount" [class.paid]="isLoanCompleted(loan)" [class.pending]="!isLoanCompleted(loan)">
  ...
  styles: [`
    ...
    .card-purple .card-icon { background: #ede9fe; }
    .card-content {
      display: flex;
    ...
    .recent-item-amount.paid { color: #22c55e; }
    .recent-item-amount.pending { color: #f59e0b; }
    .empty-recent {
      text-align: center;
      padding: 2rem;
      color: #666;
...
export class DashboardComponent implements OnInit {
  ...
  summary = computed(() => {
    ...
    if (!userId) return { totalLoaned: 0, totalCollected: 0, totalPending: 0, activeCount: 0 };

    const loans = this.loanService.getByUserId(userId);
    const payments = this.paymentService.getByUserId(userId);
